...
$about_id = $_GET['id'];
$filePath 	= '../img/about_us/' . $about_id . '/';
...
if ( !empty($fileList) ) {
	...
	foreach ($fileList as $img) {
		...
		$sql = "INSERT INTO db_about_img (about_id, img_name, order_id, created) VALUE ('{$about_id}', '{$img['name']}', '{$i}', '{$updated}')";
...
foreach($FileUploader->getRemovedFiles('file') as $key=>$value) {
	$sqlDel = "DELETE FROM db_about_img WHERE about_id = '{$about_id}' AND img_name = '{$value['name']}'";
