			            ...
			            <div class="panel-body container-fluid">
			              	<form id="aboutusAdd" name="aboutusAdd" class="form-horizontal" method="post" enctype="multipart/form-data">
				                <div class="form-group" data-plugin="formMaterial">
				                  	...
				                  	<input type="text" class="form-control" id="name" name="name" value="<?php echo $service_detail->service_name; ?>" required>
				                </div>
				                <div class="form-group" data-plugin="formMaterial">
				                  	<label class="form-control-label" for="title">ชื่อ (EN)</label>
				                  	<input type="text" class="form-control" id="name" name="name" value="<?php echo $service_detail->service_name; ?>" required>
				                </div>
								<div class="form-group" data-plugin="formMaterial">
									<label class="form-control-label" for="category">หมวดหมู่การบริการ</label>
									<?php $category_list = category_list(); ?>
									<select class="form-control" name="category">
                                        <option disabled selected hidden>หมวดหมู่การบริการ</option>
									<?php foreach ($category_list as $category_detail) : ?>
										<option value="<?php echo $category_detail->id; ?>" <?php if($service_detail->category == $category_detail->id){ echo 'selected'; } ?>><?php echo $category_detail->category_name; ?></option>
									<?php endforeach ?>
									</select>
								</div>
								<div class="form-group form-material-file" data-plugin="formMaterial">
			                      	...
			                      	<input type="file" id="covImg" name="covImg" data-plugin="dropify" data-default-file="<?php echo '../img/service/' . $service_detail->id . '/' . $service_detail->img_cover; ?>" data-allowed-file-extensions="png jpg"/>
									<i class="help-block"><i>Image size: 1200x800px</i></p>
				                </div>
				                <div class="text-right">
				                	<input type="hidden" name="service_id" value="<?php echo $service_detail->id; ?>">
